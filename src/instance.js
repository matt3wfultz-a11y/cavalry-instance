...
const TEXT_PATH_ROOTS = new Set(['path', 'editablepath', 'subpath', 'points', 'textpath']);
...
function createInstance(sourceId, offsetX, offsetY) {
  ...
  // fill / stroke: enable first so sub-attributes become connectable,
  // then explicitly connect every stroke.* attribute from the source.
  api.setFill(instanceId,   api.hasFill(sourceId));
  api.setStroke(instanceId, api.hasStroke(sourceId));
  trackFillStroke(sourceId);
  for (const key of api.getAttributes(sourceId)) {
    if (key.split('.')[0].toLowerCase() !== 'stroke') continue;
    try { api.connect(sourceId, key, instanceId, key); } catch (_) {}
  }
